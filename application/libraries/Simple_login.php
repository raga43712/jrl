		...
		if (empty($ben || $mail)) {
			...
			if (($query->num_rows() == 1) || ($query2->num_rows() == 1)) {
				...
				if ($this->CI->session->userdata('terotentikasi')) {
					...
					if ($this->CI->session->userdata('akses_level') == 'GM') {
						$this->CI->session->set_flashdata('sukses', 'Selamat datang ');
						redirect('admin');
					}
					...
					if ($this->CI->session->userdata('akses_level') == 'WRG') {
						$this->CI->session->set_flashdata('sukses', 'Selamat datang ');
						redirect('admin');
					}
					if ($this->CI->session->userdata('akses_level') == 'MGR') {
						$this->CI->session->set_flashdata('sukses', 'Selamat datang ');
						redirect('admin');
					}
					if ($this->CI->session->userdata('akses_level') == 'OP') {
						$this->CI->session->set_flashdata('sukses', 'Selamat datang ');
						redirect('admin');
					}
					if ($this->CI->session->userdata('akses_level') == 'SPV') {
						$this->CI->session->set_flashdata('sukses', 'Selamat datang ');
						redirect('admin');
					}
				}
