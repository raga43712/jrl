...
class Model_melahirkan extends CI_Model
{
    ...
    public function listing()
    {
        ...
        (SELECT m_gcm.DESCRIPTION
         ...
         WHERE  m_gcm.PARAMETER = 'CD_MARRIED' AND m_gcm.CODE = STATUS)
          AS STATUS,FATHER_NAME,MOTHER_NAME");
        $this->db->from($this->table);
        $this->db->where('DATE_JOIN IS NULL');
        $this->db->where('NO_KTP IS NULL');
    ...
    {
        $this->db->where('CD_RESIDENT', $data['CD_RESIDENT']);
    ...
    public function listing_bulan($date1, $date2)
    {
        ...
        $this->db->where('DATE_BIRTH >=', $date1);
        $this->db->where('DATE_BIRTH <=', $date2);
        $this->db->where('DATE_JOIN IS NULL');
        $this->db->where('NO_KTP IS NULL');
        $this->db->order_by('DATE_BIRTH', 'ASC');
        $query = $this->db->get();
        return $query->result();
    }
}
