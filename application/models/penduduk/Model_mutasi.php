...
class Model_mutasi extends CI_Model
{
    ...
    {
        $this->db->select("CD_RESIDENT,FULL_NAME,
        NO_KK,
        DATE_BIRTH,
        PLACE_BIRTH,
        ...
        (SELECT m_gcm.DESCRIPTION
         ...
         FROM   m_gcm
         WHERE  m_gcm.PARAMETER = 'CD_MARRIED' AND m_gcm.CODE = STATUS)
          AS STATUS,DATE_MUTATION,REASON_MUTATION");
        $this->db->from($this->table);
        $this->db->where('DATE_MUTATION IS NOT NULL');
        $this->db->or_where('DATE_MUTATION', '0000-00-00');
    ...
    public function getPenduduk()
    {
        ...
        $this->db->where('DATE_MUTATION IS NULL');
        $this->db->order_by('CD_RESIDENT', 'ASC');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function deletekonfirm($data, $id_user)
    {
        $this->db->where('CD_RESIDENT', $id_user);
        $this->db->update($this->table, $data);
    }
    ...
    public function listing_bulan($date1, $date2)
    {
        ...
        //$this->db->where('DATE_JOIN BETWEEN "' . date('Y-m-d', strtotime($date1)) . '" and "' . date('Y-m-d', strtotime($date2)) . '"');
        $this->db->where('DATE_MUTATION >=', $date1);
        $this->db->where('DATE_MUTATION <=', $date2);
        $this->db->order_by('DATE_MUTATION', 'ASC');
